...
global.markdown = function(text) {
	return smilefy(marked(text)).replace(/<img/g, '<img class="img-responsive img-rounded"').replace(/<table/g, '<table class="table table-bordered"');
};

function smilefy(str) {
	return str.replace(SMILES_REGEXP, function(match) {
		var smile = SMILES[match.replace('-', '')];
		if (smile == null)
			return match;
